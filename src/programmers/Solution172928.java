...
public class Solution172928 { // 공원을 나타내는 문자열 park, 로봇 강아지가 수행할 명령 routes
    // 명령을 수행한 뒤 놓인 위치를 세로 방향 좌표, 가로 방향 좌표 순으로 배열에 담아 return
    public int[] solution(String[] parkStr, String[] routes) {
        // 넓이와 높이 구하기
        int width = parkStr[0].length(); // 가로길이
        int height = parkStr.length;    // 세로길이
