...
public class Main2503 {
    ...
    public static int[] getCount(
            // 질문한 숫자,
            int target,
            // 백의 자리, 십의자리, 일의 자리 //-> 만든 숫자?
            int inH, int inT, int in0
    ){
        // 질문한 숫자를 각 자릿수로 분해한다
        int targetH = target / 100;
        int targetT = (target % 100) / 10;
        int target0 = target % 10;
        // 각각을 배열로 묶어본다.
        // 입력받은 숫자들
        int[] inNums = {inH,inT,in0};
        // 비교할 숫자들
        int[] targetNums = {targetH, targetT,target0};
        // 볼카운트를 저장하기 위한 변수
        int strikes = 0;
        int balls = 0;
        // 본카운트 비교하기
